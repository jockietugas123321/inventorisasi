...
$role = $_SESSION['role'] ?? '';
if (!cek_akses($role, 'pegawai', 'read')) {
...
$title = 'Data Pegawai - Inventarisasi Peralatan';
...
if ($search) {
    ...
    $where[] = "(nama_pegawai LIKE '%$searchSql%' OR username LIKE '%$searchSql%' OR jabatan LIKE '%$searchSql%')";
}
if ($filter_role) {
    $where[] = "role = '".$conn->real_escape_string($filter_role)."'";
}
$whereSql = $where ? 'WHERE ' . implode(' AND ', $where) : '';
$perPage = 10;
$page = max(1, intval($_GET['page'] ?? 1));
$offset = ($page - 1) * $perPage;
$total = $conn->query("SELECT COUNT(*) as jml FROM pegawai $whereSql")->fetch_assoc()['jml'];
$totalPages = ceil($total / $perPage);
$q = $conn->query("SELECT * FROM pegawai $whereSql ORDER BY kode_pegawai ASC LIMIT $perPage OFFSET $offset");
$roleList = $conn->query("SELECT DISTINCT role FROM pegawai WHERE role IS NOT NULL AND role != '' ORDER BY role");

$canEdit = cek_akses($role, 'pegawai', 'update');
$canDelete = cek_akses($role, 'pegawai', 'delete');
$canCreate = cek_akses($role, 'pegawai', 'create');
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        ...
        .filter-bar input, .filter-bar select { padding: 8px 12px; border-radius: 8px; border: 1px solid #bcd; font-size: 1rem; }
...
<body>
    <div class="admin-layout">
        ...
        <main class="admin-content">
            ...
            <div class="desc">Tabel data pengguna sistem beserta role, termasuk admin, satker, dan kepala bagian.</div>
            <a href="tambah.php" style="margin-bottom:18px;display:inline-block;background:#0E5C71;color:#fff;padding:8px 18px;border-radius:8px;text-decoration:none;font-weight:600;">+ Tambah Pegawai</a>
            <form class="filter-bar" method="get" action="">
                <input type="text" name="search" placeholder="Cari nama/username/jabatan..." value="<?= htmlspecialchars($search) ?>">
                <select name="role">
                    <option value="">Semua Role</option>
                    <?php while($r = $roleList->fetch_assoc()): ?>
                        <option value="<?= htmlspecialchars($r['role']) ?>" <?= $filter_role==$r['role']?'selected':'' ?>><?= htmlspecialchars($r['role']) ?></option>
                    <?php endwhile; ?>
                </select>
                ...
                <?php endif; $no = $offset+1; while ($row = $q->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if ($canEdit): ?>
                            <a href="edit.php?kode=<?= urlencode($row['kode_pegawai']) ?>" class="action-btn action-edit">Edit</a>
                            <?php else: ?>
                            <a href="#" class="action-btn action-edit" onclick="aksesDitolakSwal();return false;">Edit</a>
                            <?php endif; ?>
                            <?php if ($canDelete): ?>
                            <a href="#" class="action-btn action-del" onclick="hapusPegawaiSwal('<?= htmlspecialchars($row['kode_pegawai']) ?>');return false;">Hapus</a>
                            <?php else: ?>
    ...
    <script>
    function hapusPegawaiSwal(kode) {
        Swal.fire({
